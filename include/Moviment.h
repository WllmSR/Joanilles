...
class Moviment {
private:
    std::vector<std::vector<Posicio>> recorregut;
...
public:
    ...
    void actualitzaMoviments(std::vector<std::vector<std::pair<int,int>>>, const Posicio& original);
        ...
        bool esValid(int fila, int columna) { return (fila <= 7 && fila >= 0) && (columna <= 7 && fila >= 0);}
    std::vector<std::vector<Posicio>> getTotalRecorregut() const{ return recorregut; }
